...
int main() {
	...
	new_token_id = 257; // Reset new_token_id to 257

	uint64_t most_frequent_pair = hash_table();
	merges.push_back(most_frequent_pair);
	vocab.emplace(vocab[most_frequent_pair >> 32] + vocab[most_frequent_pair & 0xFFFFFFFF], new_token_id++);
	return 0;
}